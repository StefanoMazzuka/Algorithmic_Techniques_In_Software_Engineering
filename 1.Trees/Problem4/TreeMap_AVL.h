...
public:
    ...
    struct clave_valor {
        ...
        Valor valor;
		int tam_i;
        //clave_valor(Clave const& c, int const& tam_i, Valor const& v = Valor()) : clave(c), tam_i(tam_i), valor(v) {};
		clave_valor(Clave const& c, Valor const& v = Valor()) : clave(c), tam_i(0), valor(v) {};
    };
protected:
    ...
    /*
     ...
     punteros al hijo izquierdo y al hijo derecho, la altura y tam_i (numero nodos).
     */
    struct TreeNode;
    using Link = TreeNode *;
...
public:
	...
	/*
			...
			COSTE: O(log n) porque solo se mete por las ramas que sean necesarias
			y no recorre todo el arbol
	*/


	void KesimioElemento(map<int, int> const& arbol, int numPosiciones, int* vector) {

		int pos = 0;
		bool encontrado = false;

		rellenarTam_i(arbol.raiz,pos);

		for (int i = 0; i < numPosiciones; i++){
			KesimioElementoAux(arbol.raiz, vector[i], encontrado); 
	...
	void KesimioElementoAux(Link a, int pos, bool& encontrado ){

		if (a != nullptr && !encontrado) {

			int tamNodo = a->cv.tam_i; 

			if (tamNodo < pos)
				KesimioElementoAux(a->dr, pos, encontrado);
				...
				KesimioElementoAux(a->iz, pos, encontrado);
	...
	/*
		colocar el valor al atributo am_i
		coste O(n) porque pasamso por todos los nodos del arbol
	*/
	void rellenarTam_i(Link a, int& pos){
		
		if (a != nullptr) {

			rellenarTam_i(a->iz, pos);
			a->cv.tam_i = pos + 1;
			pos++;

			rellenarTam_i(a->dr,pos);
...
protected:
	...
	*/
    ...
    void inserta(clave_valor const& cv, Link & a) {
        ...
        else if (menor(cv.clave, a->cv.clave)) { //si la nueva clave en menor que la raiz ==> izquierda
            inserta(cv, a->iz);
            reequilibraDer(a);
        }
        else if (menor(a->cv.clave, cv.clave)) {//si la nueva clave en mayor que la raiz ==> derecha
            inserta(cv, a->dr);
            reequilibraIzq(a);
        }
